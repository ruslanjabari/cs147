...
export default function OtherProfileHeader({ userName }) {
  ...
  const [following, setFollowing] = useState(userDetails.following.includes(userName));
  return (
        ...
        <View style={styles.headerRight}>
          ...
          <View style={styles.headerRightGroup}>
            <Pressable
              style={styles.button}
              onPress={() => {
                if (!userDetails.following.includes(userName)) {
                  setUserDetails({ ...userDetails, ...userDetails['following'].push(userName) });
                  setFollowing(true);
                } else {
                  setUserDetails({
                    ...userDetails,
                    ...userDetails['following'].splice(
                      userDetails['following'].indexOf(userName),
                      1
                    ),
                  });
                  setFollowing(false);
